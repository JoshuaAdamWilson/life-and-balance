...
const Post = () => {
    ...
    const getPost = async () => {
      try {
        const response = await axios.get(
          "https://life-and-balance.herokuapp.com/api/posts/" + path,
        );
  ...
  return (
    <div className="single-post-container">
      <img className="single-blog-post-picture" src={post.photo} alt="post" />
      {updateMode ? (
        <h2>
          <input
            type="text"
      ...
      {post.username === user?.username && (
        <>
          <i className="icon far fa-edit" onClick={openUpdate}></i>
